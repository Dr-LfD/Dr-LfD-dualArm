(define (domain open-world-tamp)
  ...
  (:action learnedPick_0
    ...
    :precondition (and (robot1 ?arm) (cup ?obj) (robot1_grasp_cup ?sk)
  ...
  (:action BiOperation_1
    :parameters (?a1 ?a2 ?sk ?q1 ?q2 ?lstate ?o1 ?g1 ?g2)
    :precondition (and
      ...
      (AtGrasp ?a2 ?o1 ?g1)
      (ArmEmpty ?a1)
      (ImitateGrasp ?sk ?a1 ?o1 ?g2)
      (not (UnsafeBiOp ?a1 ?a2 ?q1 ?q2 ?sk))
    )
    :effect (and
      (DoneSkill ?sk)
      (not (AtGrasp ?a2 ?o1 ?g1))
      (ArmEmpty ?a2)
      (AtGrasp ?a1 ?o1 ?g2)
      (not (ArmEmpty ?a1))
      (CanMove ?a1)
  ...
  (:action learnedPlace_2
    :parameters (?arm ?obj ?g ?sk ?p ?s ?sp ?aq1 ?aq2 ?at)
    :precondition (and
      (robot0 ?arm) (cup ?obj) (robot0_place_cup ?sk)
      (PlanArmGripper ?arm ?s ?sk ?sp ?aq1 ?aq2 ?at)
    ...
    :effect (and (ArmEmpty ?arm) (CanMove ?arm)
                  (not (AtGrasp ?arm ?obj ?g)) (not (Localized ?obj))
