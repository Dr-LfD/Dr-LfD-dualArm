(define (stream open-world-tamp)
  ...
  (:stream sample-grasp-traj_0
    :inputs (?a ?o ?p ?sk) 
    :domain (and (Arm ?a) (robot1 ?a) (cup ?o) (Pose ?o ?p) (Graspable ?o) (robot1_grasp_cup ?sk))
  ...
  (:stream sample-biop-keypose_1
    :inputs (?a1 ?a2 ?sk ?o1 ?p1)
    :domain (and (robot0 ?a1) (robot1 ?a2) (Skillbimanual ?sk) (bimanual_0 ?sk) (cup ?o1) (Pose ?o1 ?p1) (Graspable ?o1))
    :outputs (?lc1 ?lc2 ?effGeom ?g1)
    :certified (and
      (ImitateConf ?sk ?a1 ?lc1) (Conf ?a1 ?lc1)
      (ImitateConf ?sk ?a2 ?lc2) (Conf ?a2 ?lc2)
      (GeomState ?sk ?effGeom)
      (ImitateGrasp ?sk ?a1 ?o1 ?g1)
      (Grasp ?a1 ?o1 ?g1))
  )

  (:stream test-cfree-bioperation-pose_1
  ...
  (:stream sample-place-traj_2
    :inputs (?a ?o ?s ?sp ?sk ?g)
    :domain (and (Arm ?a) (robot0 ?a) (cup ?o) (Grasp ?a ?o ?g) (table ?s) (Region ?s) (Pose ?s ?sp) (robot0_place_cup ?sk))
    :outputs (?lp ?aq1 ?aq2 ?at)
    :certified (and
      (ImitatePose ?sk ?o ?lp) (Pose ?o ?lp)
      (Conf ?a ?aq1) (Conf ?a ?aq2)
      (PlanArmGripper ?a ?s ?sk ?sp ?aq1 ?aq2 ?at)
      (Traj ?a ?at))
  )
