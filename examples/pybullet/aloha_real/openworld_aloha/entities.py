...
from examples.pybullet.aloha_real.openworld_aloha.simulation_utils import get_rigid_ancestor

from examples.pybullet.aloha_real.openworld_aloha.grasping import mesh_from_obj
...
class Object(object):
    def __init__(
        ...
        **kwargs
        ...
        self.pc_normalized = pc_normalized
        ...
        # if not robot and do not have points,generate points from mesh 
        if  manual_compute_points:
            vertices, faces = mesh_from_obj(self, client=self.client)
            obj_mesh  = trimesh.Trimesh(vertices, faces)
            points = trimesh.sample.sample_surface(obj_mesh, 1000)[0]
            self.labeled_points = [LabeledPoint(point=np.array(point), color=color, \
                                                label=self.category) for point in points]
            print('converted mesh to points: ', self.name)
    ...
    def __eq__(self, other):
        ...
        try:
            ...
            ## Returning NotImplemented is the Python convention for "can't compare with this type." Python then tries str.__eq__(entity) → also NotImplemented → falls back to identity comparison → entity is '*' → False → entity != WILD → True → entity is correctly classified as a constant. 
    ...
    @property
    def points(self):
        """Object-frame points when pc_normalized=True, else world-frame."""
    ...
    def get_world_labeled_points(self):
        if not self.labeled_points:
            return []
        if self.pc_normalized:
            return [
                LabeledPoint(tform_point(self.observed_pose, lp.point), lp.color, lp.label)
                for lp in self.labeled_points
            ]
        return list(self.labeled_points)

    def get_world_points(self):
        return [lp.point for lp in self.get_world_labeled_points()]
...
class Robot(Object):
    ...
    def get_component(self, group, visual=True):
        ...
        if group not in self.components:
            ...
            set_all_color(self.components[group], BROWN)
