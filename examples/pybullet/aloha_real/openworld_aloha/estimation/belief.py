...
from examples.pybullet.utils.pybullet_tools.voxels import VoxelGrid
from scipy.optimize import linear_sum_assignment

from examples.pybullet.aloha_real.openworld_aloha.estimation.clustering import relabel_clusters
from examples.pybullet.aloha_real.openworld_aloha.estimation.geometry import estimate_surface_mesh, trimesh_from_body
from examples.pybullet.aloha_real.openworld_aloha.estimation.observation import (
    LabeledPoint,
    aggregate_color,
...
from examples.pybullet.aloha_real.openworld_aloha.estimation.surfaces import plane_from_pose, surface_point_filter
from examples.pybullet.aloha_real.openworld_aloha.entities import (
    ENVIRONMENT,
    TABLE,
    UNKNOWN,
    Object,
...
from .pc_utils import  merge_cluster_by_label, filter_pc


GRASP_EXPERIMENT = True

if GRASP_EXPERIMENT:
    FRAGILE_HEIGHT = 5e-2  # Tall fragile
else:
...
def _center_cluster_to_body(cluster, body, client=None):
    body_pose = get_pose(body, client=client)
    return tform_labeled_points(invert(body_pose), cluster)
...
class EstimatedObject(Object):
    def __init__(
        ...
        **kwargs
        ...
        # self.initial_pose = get_pose(body)  
        # self.pc_pose = self.observed_pose
        # TODO: store OOBB
        if (color is None) and self.labeled_points:
            ...
            set_all_color(self.body, apply_alpha(color, alpha=0.7), client=self.client)
    ...
    def update(self, newcloud, tform, camera_image, min_points, surface, **kwargs):
        ...
        self.body = body
        self.pc_normalized = False
        color = aggregate_color(self.labeled_points)
        set_all_color(self.body, apply_alpha(color / 255.0, alpha=0.7))
        self.color = color  # TODO: could extract from the body/texture instead
        self.name = "{}#{}".format(self.category, self.body)
        return True
    
    def only_update_pc(self, new_body, new_labeled_points):

        self.remove()

        self.labeled_points = new_labeled_points
        self.body = new_body
        self.pc_normalized = True
        self.name = "{}#{}".format(self.category, self.body)
        return True
...
class Belief(object):
    def __init__(
        ...
        **kwargs
        ...
        if ws_aabb is None: # default workspace
            ws_lower = np.array([-0.35, -0.3, -0.03])
            ws_upper = np.array([0.35, 0.40, 0.45])
            self.workspace_aabb = AABB(lower=ws_lower, upper=ws_upper)
        else:
            self.workspace_aabb = ws_aabb
            print("Workspace AABB: ", self.workspace_aabb)
    ...
    def estimate_objects(
        ...
        **kwargs
        ...
        # exclude points from unknown, table, output 2d image
    def get_labeled_cloud(self, camera_image, surface = None, max_depth=float("inf"), filter_surface = True,  **kwargs):
        pixels = [
            pixel
            for pixel in iterate_image(camera_image, step_size=2)
            if is_object_label(camera_image.segmentationMaskBuffer[pixel])
        ]
        # exclude far away points, output point in the world frame
        relevant_cloud = [
            lp
        ...
        # filter out surface points, e.g., reference lines
        if surface is not None and filter_surface:  # TODO speedup todo
    ...
    def get_clusters_w(self, camera_image, surface=None, min_points = 30, filter_outliers = True, draw_aabb_color = None, **kwargs):
 
        relevant_cloud = self.get_labeled_cloud(camera_image, surface=surface, **kwargs)

        clusters = relabel_clusters(relevant_cloud)
        clusters = sorted(
            clusters, key=lambda c: min(lp.point[0] for lp in c)
        )  # TODO: relative to the robot
        cluster_aabbs = [aabb_from_points([lp.point for lp in cluster]) for cluster in clusters]
        labeled_clusters = {}
        for i, cluster in enumerate(clusters):
            ...
            if filter_outliers:
                cluster_aabb = cluster_aabbs[i]
                if not aabb_contains_aabb(cluster_aabb, self.workspace_aabb):
                    continue
            
            if draw_aabb_color is not None:
                draw_aabb(cluster_aabb, color=COLOR_FROM_NAME['draw_aabb_color'])

            label = cluster[0].label
            labeled_clusters[label] = cluster

        return labeled_clusters


    
    def estimate_objects_multiview(self, camera_images, use_seg = True, surface=None,         min_points=30,
        save_relabeled_img=False,
        surfaces_movable=True,
        filter_outliers=True,
        ...
        **kwargs
        ...
        base_img = camera_images[0]
        self.observations.append(base_img)

        observed_obj_clusters = {}
        for composed_img in camera_images:
            labeled_clusters = self.get_clusters_w(composed_img, surface=surface, min_points=min_points, filter_outliers=filter_outliers, **kwargs)

            observed_obj_clusters = merge_cluster_by_label(observed_obj_clusters, labeled_clusters)

            
        for label, cluster in observed_obj_clusters.items():
            if debug_save_name is not None:
                # save the cluster point cloud (world-frame points for debugging)
                import open3d as o3d
                import os
                pcd = o3d.geometry.PointCloud()
                pcd.points = o3d.utility.Vector3dVector([lp.point for lp in cluster])
                pcd.colors = o3d.utility.Vector3dVector([lp.color for lp in cluster])
                file_name = debug_save_name + f'_{label.category}.ply'
                o3d.io.write_point_cloud(file_name, pcd)

            # ## use DBSCAN to filter the points
            # body_points = [lp.point for lp in cluster]
            # filtered_points = filter_pc(body_points)
            # filtered_cluster = [LabeledPoint(point, cluster[0].color, cluster[0].label) for point in filtered_points]
            ...
            # Transform labeled points into the body-local frame so that the stored
            # labeled_points match the object-frame convention used in simulation.
            centered_cluster = _center_cluster_to_body(cluster, body, client=self.client)
            ...
            if surfaces_movable or not is_surface(cluster):
                ...
                self.estimated_objects.append(
                    EstimatedObject(
                        ...
                        labeled_points=centered_cluster,
        ...
        if save_relabeled_img:
            save_image_from_cluster(observed_obj_clusters.values(), base_img, min_points=min_points)
    ...
    def estimate_objects(
        ...
        **kwargs
        ...
        # exclude points from unknown, table, output 2d image
        relevant_cloud = self.get_labeled_cloud(camera_image, surface=surface, max_depth=max_depth, **kwargs)
        ...
        for i, cluster in enumerate(clusters):
            ...
            if filter_outliers:
                ...
                # ## filter out the markers on the floor
                # if cluster_aabb.upper[2]<0.07:
                #     continue

            draw_aabb(cluster_aabb)
            ...
            # after surface estimation, add the mesh points to the cluster
            if add_meshpts:
                added_mesh = trimesh_from_body(body)
                added_pc = added_mesh.sample(1000)   # in the object-centric frame
                added_color = cluster[0].color
                added_label = cluster[0].label
                added_pc = [LabeledPoint(point, added_color, added_label) for point in added_pc]
                #TODO: merge it with the labeled points
                body_pose = get_pose(body)
                tformed_added_pc = tform_labeled_points(body_pose, added_pc)
                cluster += tformed_added_pc
