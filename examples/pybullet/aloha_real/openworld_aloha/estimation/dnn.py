...
from examples.pybullet.utils.pybullet_tools.utils import implies
from examples.pybullet.aloha_real.openworld_aloha.estimation.belief import take_ml_estimate
from examples.pybullet.aloha_real.openworld_aloha.entities import BG, BOWL, CUP, OTHER, TABLE, UNKNOWN
from examples.pybullet.aloha_real.openworld_aloha.workers.sam3_mask_client import (
    get_shared_sam3_client,
)
from examples.pybullet.aloha_real.openworld_aloha.estimation.lis import (
    CKPT_PATH,
    DET_CKPT_PATH,
    DEVICE,
    DSN_CONFIG,
...
# DEFAULT_SAM3_PATH = os.path.expanduser("~/yzchen_ws/perception/sam3")
...
def sam3_str_seg_from_masks(image_shape, masks_by_object):
    str_seg = np.full(tuple(image_shape) + (len(DEFAULT_VALUE),), DEFAULT_VALUE, dtype=object)
    instance_idx = 1
    for object_name, masks in masks_by_object.items():
        for mask in masks:
            ...
            str_seg[mask_bool, 0] = object_name
            str_seg[mask_bool, 1] = INSTANCE_TEMPLATE.format(instance_idx)
            instance_idx += 1
    return str_seg
...
def masks_objects_to_str_seg(masks, objects, image_shape, default_value=DEFAULT_VALUE):
    str_seg = np.full(image_shape + (2,), default_value, dtype=object)
    for i, (mask, obj_label) in enumerate(zip(masks, objects)):
        mask_arr = np.asarray(mask, dtype=bool)
        if mask_arr.ndim == 3:
            mask_arr = mask_arr.squeeze(0)
        str_seg[mask_arr, 0] = obj_label
        str_seg[mask_arr, 1] = f"instance_{i + 1}"
    return str_seg


class SAM(CategoryAgnosticSeg):
    def __init__(self,text_prompt = 'tape.', sam_path = None, use_server = False, **kwargs):
        self.use_server = use_server
        self.object_names = text_prompt.split('.')
        self.object_names = [obj for obj in self.object_names if obj != '']
        super().__init__()
        self.base_path = sam_path
        self.text_prompt = text_prompt
        self._grounded_sam_segmenter = None
        if not self.use_server:
            from examples.pybullet.aloha_real.openworld_aloha.contact_monitoring import (
                SegReuseRegistry,
            )

            self._grounded_sam_segmenter = SegReuseRegistry.get_grounded_sam_segmenter()
    ...
    def get_seg(
        ...
        **kwargs
        ...
        if self.use_server:
            return self.get_seg_from_file(rgb_image, **kwargs)

        rgb = np.asarray(rgb_image, dtype=np.uint8)
        masks_list = []
        objects_list = []
        for name in self.object_names:
            mask_seq = self._grounded_sam_segmenter.segment_sequence(
                rgb[np.newaxis], name
            )
        ...
        return masks_objects_to_str_seg(
            masks_list, objects_list, rgb.shape[:2], DEFAULT_VALUE
        )

    ## TODO: below only works for 1 object!
    def get_seg_from_file(self, rgb_image, cam_dir = None, **kwargs):
        import cv2, os, rospy
        ...
        try:
            ...
            str_seg = np.full(img_shape + (2,), DEFAULT_VALUE , dtype=object)

            ## find '*_mask.png' files in the cam_dir
            import fnmatch
            mask_files = fnmatch.filter(os.listdir(cam_dir), '*_mask.png')

            for i, mask_file in enumerate(mask_files):
            # for i in range(len(self.object_names)):
                # mask_path = os.path.join(cam_dir, self.object_names[i]+'_mask.png')  
                mask_path = os.path.join(cam_dir, mask_file)
                print('Reading binary mask from file '+ mask_path)

                obj_name = mask_file.split('_mask.png')[0]

                seg_mask_255 = cv2.imread(mask_path,  cv2.IMREAD_GRAYSCALE) 
                if seg_mask_255 is None:
                    print('Error reading mask from file '+ mask_path)
                    return None
                
                seg_mask = seg_mask_255.astype(bool)
                if seg_mask is None:
                    raise ValueError()
                for r, c in iterate_array(str_seg, dims=[0, 1]):
                    if seg_mask[r, c] != False:
                        # str_seg[r, c, 0] = self.object_names[i]
                        str_seg[r, c, 0] = obj_name
                        str_seg[r, c, 1] = 'instance_'+str(i+1)
...
class SAM3(CategoryAgnosticSeg):
    def __init__(
        self,
        text_prompt="tape.",
        sam3_path=None,
        ...
        **kwargs,
        ...
        super().__init__()
        self.object_names = parse_text_prompt_objects(text_prompt)
        self.base_path = os.path.abspath(os.path.expanduser(sam3_path))
        self.model_dir = sam3_model_dir
        self.checkpoint_path = sam3_checkpoint
        self.conda_env = sam3_conda_env
        from examples.pybullet.aloha_real.openworld_aloha.contact_monitoring import (
            SegReuseRegistry,
        )

        self._client = SegReuseRegistry.get_sam3_client()

    def get_seg(self, rgb_image, **kwargs):
        rgb = np.asarray(rgb_image)
        masks_by_object = self._client.segment(rgb, self.object_names)
        return sam3_str_seg_from_masks(rgb.shape[:2], masks_by_object)
