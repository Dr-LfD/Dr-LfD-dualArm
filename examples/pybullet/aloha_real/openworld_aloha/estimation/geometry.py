...
from examples.pybullet.aloha_real.openworld_aloha.estimation.bounding import convex_hull_2d, estimate_oobb, min_circle
from examples.pybullet.aloha_real.openworld_aloha.estimation.completion import filter_visible, refine_shape
from examples.pybullet.aloha_real.openworld_aloha.estimation.concave import concave_mesh, create_concave_mesh
from examples.pybullet.aloha_real.openworld_aloha.estimation.observation import aggregate_color, tform_labeled_points, draw_points
from examples.pybullet.aloha_real.openworld_aloha.estimation.surfaces import Plane, point_plane_distance
from examples.pybullet.aloha_real.openworld_aloha.grasping import mesh_from_obj
from examples.pybullet.aloha_real.openworld_aloha.simulation_utils import  Z_AXIS
from examples.pybullet.aloha_real.openworld_aloha.entities import BOWL
...
def trim_mesh(submesh, trim=False):
    ...
    for plane in planes:
        ...
        vertices, faces, uvs = slice_faces_plane(vertices, faces, *plane)
        submesh = Mesh(vertices, faces)
...
def estimate_mesh(
    ...
    **kwargs
    ...
    if(any([(aabb.upper[i]-aabb.lower[i]) > \
        max_aspect_xy for i in range(2)])):
        return None
    if((aabb.upper[2]-aabb.lower[2]) > max_aspect_z):
        return None
    if (
        (len(points) < 3)
...
def estimate_surface_mesh(
    ...
    body = estimate_mesh(
        labeled_cluster, camera_image=camera_image, mass = 0.2,  **kwargs
