...
from examples.pybullet.aloha_real.openworld_aloha.estimation.clustering import cluster_trimesh
...
from examples.pybullet.aloha_real.openworld_aloha.estimation.surfaces import (
    Plane,
    compute_inliers,
    create_surface,
    plane_from_pose,
...
from examples.pybullet.aloha_real.openworld_aloha.entities import TABLE, UNKNOWN, Label, Table
from examples.pybullet.aloha_real.openworld_aloha.simulation_utils import (
    Z_AXIS,
    find_closest_color,
    get_color_distance,
    mean_hue,
...
def estimate_region(
    ...
    **kwargs
    ...
    # from examples.pybullet.utils.pybullet_tools.utils import rectangular_mesh, create_rectangular_surface
...
def estimate_regions(
    ...
    **kwargs
    ...
    pos_table_points_by_point = {}
    for lp in pos_table_points:
        pos_table_points_by_point.setdefault(tuple(lp.point), lp)
    extracted_pos_table_points = list(pos_table_points_by_point.values())
    extracted_pos_table = [lp.point for lp in extracted_pos_table_points]
    table_plane, _inliers = ransac_estimate_plane(
        extracted_pos_table, threshold=plane_threshold, max_error=math.radians(1)
    )
    if table_plane is None:
        raise ValueError( "Failed to estimate table plane from extracted points. ""Check point cloud frame/scale and plane_threshold.")
    ...
    # if graspdata_dir is not None:
    #     import open3d as o3d
    #     import os
    #     pcd = o3d.geometry.PointCloud()
    #     pcd.points = o3d.utility.Vector3dVector([lp.point for lp in pos_table_points])
    #     pcd.colors = o3d.utility.Vector3dVector([lp.color for lp in pos_table_points])
    #     file_name = os.path.join(graspdata_dir, 'scene_{}.ply'.format(episode_idx))
    #     o3d.io.write_point_cloud(file_name, pcd)


    # import open3d as o3d
    # pcd = o3d.geometry.PointCloud()
    # pcd.points = o3d.utility.Vector3dVector(extracted_pos_table)
    # pcd.colors = o3d.utility.Vector3dVector(
    #     [remove_alpha(lp.color) for lp in extracted_pos_table_points]
    # )
    # o3d.io.write_point_cloud('all_pc_world.ply', pcd)
    # if save_relabled:
...
def estimate_surfaces(belief, camera_image, **kwargs):
    existing_table = next(
        (
            surface
            for surface in belief.known_surfaces
            if getattr(surface, "category", None) == "table"
        ),
        None,
    )
    if existing_table is not None:
        other_surfaces = [
            surface for surface in belief.known_surfaces if surface is not existing_table
        ]
        belief.known_surfaces = [existing_table] + other_surfaces
        return belief.known_surfaces
