...
from examples.pybullet.aloha_real.openworld_aloha.estimation.surfaces import (get_plane_quat)
# from examples.pybullet.aloha_real.openworld_aloha.simulation.control import interpolate_controller
from examples.pybullet.aloha_real.openworld_aloha.simulation_utils import X_AXIS, Z_AXIS
...
PREGRASP_DISTANCE = 0.05  # 0.05 | 0.07 |0.10 
...
def get_pregrasp(
    ...
    robot_name = None, **kwargs
):
    # tool_distance is toward x axis of the tool, (x: aloha, z:franka)object_distance is toward -z axis of the object
    if 'aloha' in robot_name:
        offset_displacement = Pose(Point(x=tool_distance))
    elif 'panda' in robot_name:
        offset_displacement = Pose(Point(z=tool_distance))
    else:
        raise NotImplementedError(robot_name)
    pregrasp_pose = multiply(
        gripper_from_tool,
        offset_displacement,
        grasp_tool,
        Pose(Point(z=-object_distance)),
    )
    # draw_pose(invert(pregrasp_pose), length=0.15)
