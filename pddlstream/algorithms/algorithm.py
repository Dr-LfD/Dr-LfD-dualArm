...
def parse_constants(domain, constant_map):
    ...
    for constant in domain.constants:
        ...
        if isinstance(value, Object) and getattr(value, 'pddl', None) == constant.name:
            obj_from_constant[constant.name] = value
        else:
            obj_from_constant[constant.name] = Object(value, name=constant.name) # TODO: remap names
        # TODO: add object predicate

    # Re-register so stream parsing (Object.from_name) can resolve constants after reset_globals()
    for name, obj in obj_from_constant.items():
        Object._obj_from_name[name] = obj
