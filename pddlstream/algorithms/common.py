...
StoredPlan = namedtuple('StoredPlan', ['plan', 'cost', 'time'])
...
class SolutionStore(object):
    ...
    def add_plan(self, plan, cost):
        ...
        if is_plan(plan) and (cost < self.best_cost):
            self.solutions.append(StoredPlan(plan, cost, elapsed_time(self.start_time)))
...
# evaluations is from the history. if evaluation_from_fact(fact) is in evaluations, no need to add it.
