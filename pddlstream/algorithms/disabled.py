...
def bind_action_plan(opt_plan, mapping):
    ...
    return OptPlan(new_action_plan, new_preimage_facts,opt_plan.action_plan_instances)


def bind_stream_result(result, mapping):
    bound_result = result.remap_inputs(mapping)
    if not hasattr(result, 'output_objects'):
        return bound_result
    output_objects = apply_mapping(result.output_objects, mapping)
    return result.__class__(
        bound_result.instance,
        output_objects,
        result.opt_index,
        result.call_index,
        result.list_index,
...
def bind_stream_plan(stream_plan, mapping):
    if not is_plan(stream_plan):
        return stream_plan
    return [bind_stream_result(result, mapping) for result in stream_plan]
...
def process_instance(store, domain, instance, disable=False):
    ...
    new_results, new_facts = instance.next_results(verbose=store.verbose) # sampled values, certified predicates
...
def process_stream_plan(store, domain, disabled, stream_plan, action_plan, cost,
    ...
    if (num_wild == 0) and (len(stream_plan) == len(bound_plan)):
        bound_action_plan = bind_action_plan(action_plan, bindings)
        store.add_plan(bound_action_plan, cost)
