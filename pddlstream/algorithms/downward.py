...
def find_translate_path(fd_path):
    build_translate_path = os.path.join(find_build(fd_path), 'bin/translate')
    if os.path.exists(os.path.join(build_translate_path, 'instantiate.py')):
        return build_translate_path
    src_translate_path = os.path.join(fd_path, 'src/translate')
    if os.path.exists(os.path.join(src_translate_path, 'instantiate.py')):
        return src_translate_path
    raise RuntimeError('Could not find FastDownward translator sources')
...
TRANSLATE_PATH = find_translate_path(FD_PATH)
