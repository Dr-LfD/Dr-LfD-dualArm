...
##################################################
# The function begins by initializing several variables and parsing the problem using the parse_problem function. It then enters a while loop, which continues until the problem is solved, the maximum number of iterations is reached, or the complexity limit is exceeded.

# Within the loop, the function first processes any eager externals, which are streams that should be evaluated as soon as possible. It then prints some information about the current state of the planning process.

# Next, the function attempts to solve the problem optimistically, meaning it assumes that all stream outputs will be successful. If the problem cannot be solved optimistically, the complexity limit is increased and the loop continues.

# If a plan is found, it is processed and added to a queue of skeletons, which are partial plans that can be expanded into full plans. The function then attempts to process the skeleton queue.

# Finally, once the loop ends, the function prints a summary of the planning process and returns the best solution found.

# The PlanConstraints class is used to specify constraints on the plan, such as the maximum cost. The set and list classes are standard Python data structures, and the filter class is a Python built-in function for filtering elements from an iterable. 
# The bool class is a built-in Python class for boolean values. The SolutionStore class is used to store and manage the solutions found during the planning process.
...
def solve_abstract(problem, constraints=PlanConstraints(), stream_info={}, replan_actions=set(),
    ...
    while True:
        if store.is_terminated() :
            print('store is terminated')
            break
        if num_iterations >= max_iterations:
            print('num_iterations >= max_iterations')
            break
        if complexity_limit > max_complexity:
            print('complexity_limit > max_complexity')
            break
        num_iterations += 1
        ...
        stream_plan = combine_optimizers(evaluations, stream_plan)  # no use if don't optimize the order
        ...
        ################  in current iteration, not all variables are grounded, i.e., lazy instance exists
        ...
        if allocated_sample_time <0:
            allocated_sample_time = (search_sample_ratio * store.search_time) - store.sample_time if len(skeleton_queue.skeletons) <= max_skeletons else INF
            print('debug: allocated_sample_time is negative')
