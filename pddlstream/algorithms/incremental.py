...
def process_instance(instantiator, store, instance, verbose=False): #, **complexity_args):
    ...
    # for s' in INSTANTIATE-SAMPLERS(), 
    # given input arguments, compute stream output values (pos, conf, etc)
    ...
    for result in new_results:
        ...
        for evaluation in add_certified(evaluations, result):
            # in solve_satisfaction(), it performs product(samples, |I|), i.e., fit sampled results for each stream with a suitable argument,
            #  in order to get new samplers from stream using _add_new_instances()
            # e.g., if the result is a conf q1, then fit it into connect(q0 q1) where q0 is already computed in prev iter,
            # and push  connect(q0 q1) to queue for further sampling.
    ...
    # adds the processed sampler instances back to queue to be used again on later iterations.
    if not instance.enumerated:
        instantiator.push_instance(instance)
    return new_results

# PROCESS-SAMPLERS()  
# core function that takes in constraints and conditional samplers, output certified sample instances
def process_stream_queue(instantiator, store, complexity_limit=INF, verbose=False):
    instances = [] # this is queue
    results = []
    num_successes = 0
...
def solve_incremental(problem, constraints=PlanConstraints(),
    ...
    # if contain no fluents, just output externals (streams)
    ...
    # queue = INSTANTIATE-SAMPLES(\psi = conditional samplers, a set of boolean predicates as constraints)
    instantiator = Instantiator(static_externals, evaluations)
    # ( in the paper, INCREMENTAL line 2, there is no process. Here is in case that one-hop sampling can drive initial scene satisfies the goal
    num_calls += process_stream_queue(instantiator, store, complexity_limit, verbose=verbose)
    # while the problem is not solve
