...
def solve_restart(problem, max_tamp_time=INF, max_restarts=0, iteration_time=INF, abort=True, **kwargs):
    ...
    for attempt in irange(1+max_restarts):
        ...
        if elapsed_time(start_time) > max_tamp_time:
            break
        if attempt >= 1:
            print(SEPARATOR)
        #solution = planner_fn(problem) # Or include the problem in the lambda
        remaining_time = min(iteration_time, max_tamp_time-elapsed_time(start_time))
        solution = solve(problem, max_time=remaining_time, **kwargs)
        if solution is None:
            print('No solution found for TAMP!')
            continue
    ...
    return Solution(None, INF, certificate, None)
