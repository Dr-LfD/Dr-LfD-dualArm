...
from pddlstream.algorithms.disabled import process_instance, update_bindings, update_cost, bind_action_plan, bind_stream_plan
...
class SkeletonQueue(Sized):
    ...
    def is_active(self):
        ...
        return self.queue and (not self.store.is_solved())
    ...
    def _process_binding(self, binding):
        ...
        readd = not instance.enumerated # if instance can still generate new bindings, re-add it to queue
        return readd, is_new
    ...
    def process_root(self):
        ...
        readd, is_new = self._process_binding(binding) # is_new: if new bindigns are generated
    ...
    def greedily_process(self):
        ...
        while self.is_active():
            priority, binding = self.peak_binding()  # s=POP(queue)
            if not binding.is_greedy(): #priority.not_greedy:
                break
            num_new += self.process_root()  # elements += PROCESS(s)
        return num_new
    ...
    def timed_process(self, max_time=INF, max_iterations=INF):
        ...
        if max_time <0:
            print('debug: max_time is negative')
    ...
    def process(self, stream_plan, action_plan, cost, complexity_limit, allocated_sample_time=0, accelerate=False):
        ...
        used_time = elapsed_time(start_time)
        self.timed_process(max_time=(allocated_sample_time - used_time))
